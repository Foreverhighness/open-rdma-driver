...
fn write_packet(
    ...
    // byte.
    #[allow(clippy::cast_possible_truncation)]
    ip_header.set_total_length(u32::from_be_bytes([0, 0, ACKPACKET_SIZE_WITHOUT_MAC as u8, 0]));
    ...
    let aeth_hdr_buf = &mut mac_header.0[MAC_HEADER_SIZE + IPV4_HEADER_SIZE + UDP_HEADER_SIZE + BTH_HEADER_SIZE..];
    ...
    let mut nreth_header = NReth(
        &mut mac_header.0[MAC_HEADER_SIZE + IPV4_HEADER_SIZE + UDP_HEADER_SIZE + BTH_HEADER_SIZE + AETH_HEADER_SIZE..],
    );
    if is_nak {
...
mod tests {
    ...
    fn test_calculate_ipv4_checksum() {
        ...
        let ref_header: [u8; 20] = [
            0x45, 0x00, 0x00, 0x34, 0xb4, 0xef, 0x40, 0x00, 0x25, 0x06, 0x00, 0x00, 0x14, 0x59, 0xed, 0x08, 0xac, 0x1b,
            0xe8, 0xda,
        ];
        ...
        let ref_header = [
            0x45, 0x00, 0x02, 0x0c, 0x3f, 0x3b, 0x40, 0x00, 0x29, 0x06, 0x00, 0x00, 0x8c, 0x52, 0x72, 0x15, 0xac, 0x1b,
            0xe8, 0xda,
        ];
        ...
        assert_eq!(checksum, expected_checksum);
        let ref_header = [
            0x45, 0x0, 0x0, 0xbc, 0x0, 0x1, 0x0, 0x0, 0x40, 0x11, 0x0, 0x0, 0xc0, 0xa8, 0x0, 0x2, 0xc0, 0xa8, 0x0, 0x3,
        ];
        let checksum = calculate_ipv4_checksum(&ref_header);
