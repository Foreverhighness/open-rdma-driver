...
use crate::device::software::emulator::net::util::{generate_ack, message_to_bthreth, message_to_imm_dt};
...
use crate::device::ToHostWorkRbDescStatus;

#[derive(Debug)]
pub struct WriteOnlyWithImmediate<'msg> {
    // TODO(fh): replace with BaseTransportHeader
    bth: &'msg RdmaMessage,
    reth: &'msg RdmaMessage,
}
type Message<'msg> = WriteOnlyWithImmediate<'msg>;
...
impl<UA: Agent, DC: Client> HandleMessage<Message<'_>> for DeviceInner<UA, DC> {
    fn handle(&self, msg: Message, src: core::net::IpAddr) -> crate::device::software::emulator::Result {
        let msg = msg.bth;
        ...
        let need_report_header = true;
        if need_report_header {
            let descriptor0 = message_to_bthreth(msg, expected_psn_option.unwrap_or(0), req_status, can_auto_ack);
