...
use crate::net::util::{generate_ack, message_to_bthreth, message_to_imm_dt};
...
impl<UA: Agent, DC: Client> HandleMessage<Message<'_>> for DeviceInner<UA, DC> {
    fn handle(&self, msg: Message, src: core::net::IpAddr) -> crate::Result {
        ...
        let can_auto_ack;
        let _can_skip_report_header;
        let expected_psn_option;

        if abnormal_packet {
            can_auto_ack = false;
            _can_skip_report_header = false;
            ...
            _can_skip_report_header = psn == expected_psn;
